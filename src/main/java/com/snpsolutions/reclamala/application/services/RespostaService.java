...
public class RespostaService {
    ...
    public List<RespostaDTO> listarComentarios() {
        ...
        return comentarios.stream().map(this::toRespostaDTO).collect(Collectors.toList());
    }

    public List<RespostaDTO> listarComentariosPorUsuario(Usuario usuario) {
       List<Resposta> comentarios = respostaRepository.findByUsuarioComentario(usuario);
       return comentarios.stream().map(this::toRespostaDTO).collect(Collectors.toList());
    } 
    ...
    private RespostaDTO toRespostaDTO(Resposta resposta) {
        ...
        dto.setMatriculaUsuario(resposta.getUsuarioComentario().getMatricula());
        return dto;
    }

     @Transactional
    public Resposta criarRespostaParaUmComentario(RespostaDTO respostaDTO) {
       Comentario comentario = comentarioRepository.findById(respostaDTO.getIdComentario())
        ...
        novaResposta.setUsuarioComentario(usuario);
    ...
    public Resposta atualizarRespostaDeUmComentario(Long respostaId, Resposta respostaAtualizada) {
        return respostaRepository.findById(respostaId).map(resposta -> {  
            ...
            resposta.setDataCriacaoComentario(respostaAtualizada.getDataCriacaoComentario());
    ...
    public void curtirResposta(Long respostaId) {
       respostaRepository.findById(respostaId).ifPresent(comentario -> {
        comentario.setQtdCurtidas(comentario.getQtdCurtidas() + 1);
        respostaRepository.save(comentario);
       });
    }
